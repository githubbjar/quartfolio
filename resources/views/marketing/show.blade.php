<x-layout :title="$project->title . ' (Marketing & Design Campaign) | Quartfolio'">
    ...
    <x-section.art-layout bg="graphics-bg" class="relative">
                ...
                <div class="bg-white text-black p-4 w-fit mx-auto"
                    ...
                    @if($project->quarter && $project->year)
                        <p class="tinos-regular leading-tight text-center text-sm md:text-md tracking-wide uppercase text-black">
                            Marketing & Campaigns &#8212; {{ $project->quarter_label }} {{ $project->year }}
                        </p>
                    @endif
        ...
        >
            <img
                :src="activeImage"
                @click="activeImage = hero"
                alt="{{ $project->title }}"
                class="w-full max-w-300 mx-auto h-auto pt-4 cursor-pointer"
            >

            @if(!empty($project->gallery))
                <div class="flex justify-center gap-3 mt-10 px-4 flex-wrap">
